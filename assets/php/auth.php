<?php
	...
	class Auth extends Database
	{
		...
		public function currentUser($cemail){
		...
		//Update New Password
		public function update_new_pass($hnewpass, $email){
			$sql = "UPDATE users SET token = '',password = :pass WHERE email = :email AND deleted !=0";

			$stmt = $this->conn->prepare($sql);
			$stmt->execute(['pass'=>$hnewpass,'email' =>$email]);
			return true;
		}
		...
		public function verify_email($email){
			$sql = "UPDATE users SET verifie =1 WHERE email = :email AND deleted != 0";
			$stmt = $this->conn->prepare($sql);
			$stmt-> $this->conn->prepare($sql);
			$stmt->execute(['email'=>$email]);
			return true;
